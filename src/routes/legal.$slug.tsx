...
function policies(brand: string, contact: string): Record<string, Policy> {
  return {
    terms: {
      title: "Terms & Conditions",
      summary: `The rules that apply when a business uses the ${brand} payment gateway.`,
      sections: [
        {
          h: "Scope of service",
          p: `${brand} provides a hosted checkout that lets merchants receive digital asset payments from their customers. We are a technology provider, not a custodian, exchange, broker or lender.`,
        },
        {
    ...
    aml: {
      ...
      summary: "Our approach to anti-money-laundering and counter-terrorist-financing across the gateway.",
      sections: [
        {
    ...
    "customer-due-diligence": {
      ...
      summary: `How we verify the businesses that use ${brand}, and when we go further.`,
      sections: [
        {
    ...
    "risk-disclaimer": {
      ...
      summary: `Please read this before sending or accepting any digital asset payment through ${brand}. Blockchain payments are final and mistakes usually cannot be undone.`,
      sections: [
        {
          h: "Payments are irreversible",
          p: `Once a transaction is confirmed on the blockchain it cannot be recalled, cancelled, reversed or charged back — not by the customer, not by the merchant, and not by ${brand}.`,
        },
        {
          h: "A wrong address means the assets are lost",
          p: `${brand} does not control the receiving address and has no ability to freeze, reverse or restore a transfer sent to the wrong address.`,
        },
        {
          h: "A wrong network means the assets are lost",
          p: "Every payment address on the checkout is valid for one specific network only. Sending over a different network will normally result in a permanent loss.",
        },
        {
          h: "A wrong asset means the assets are lost",
          p: "A deposit address is issued for the specific coin or token selected at checkout. Sending a different asset is unrecoverable.",
        },
        {
          h: "Payment window and quoted amount",
          p: "When a coin and network are selected, the checkout quotes an exact amount and opens a payment window of 60 minutes. Funds that arrive after expiry are not automatically credited.",
        },
        {
          h: "No deposit protection",
          p: "Digital assets are not bank deposits and are not covered by any deposit-guarantee scheme.",
        },
        {
          h: "No investment, tax or legal advice",
          p: "Nothing on this website is investment, tax, accounting or legal advice.",
        },
      ],
    },
    "acceptable-use": {
      title: "Acceptable Use Policy",
      summary: `Activity that may not be processed through the ${brand} gateway.`,
      sections: [
        {
    ...
    cookies: {
      ...
      sections: [
        {
          h: "Essential storage",
          p: "We store checkout session state in your browser so the pages work correctly. This cannot be turned off without breaking the site.",
        },
        {
...
export const Route = createFileRoute("/legal/$slug")({
  loader: ({ params }) => {
    const map = policies("PGX", "support@localhost");
  ...
  head: ({ loaderData }) => {
    ...
    const policy = policies("PGX", "support@localhost")[loaderData.slug];
    return {
      meta: [
        { title: policy?.title || "Legal" },
        { name: "description", content: policy?.summary || "" },
      ],
    };
  },
  component: LegalPage,
...
function LegalPage() {
  ...
  const contact = contactEmailForBrand(brand);
  const policy = policies(brand, contact)[slug];
    ...
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-14">
        ...
        <p className="mt-12 rounded-2xl border border-border bg-card p-5 text-sm text-muted-foreground">
          {brand} · Questions about this document?{" "}
